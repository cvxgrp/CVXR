...
method(reduction_apply, SCIP_Solver) <- function(x, problem, ...) {
  ...
  ## CVXPY SOURCE: scip_conif.py lines 135-136
...
method(solve_via_data, SCIP_Solver) <- function(x, data, warm_start = FALSE, verbose = FALSE,
                                                 solver_opts = list(), ...) {
  .require_solver_package(SCIP_SOLVER)
  ...
  vtype <- rep("C", n_orig)
  ## CVXPY SOURCE: scip_conif.py lines 205-224
  lb <- data[[LOWER_BOUNDS]] %||% rep(-Inf, n_orig)
  ub <- data[[UPPER_BOUNDS]] %||% rep(Inf, n_orig)
  ...
  ## Map status
  status <- .status_from_map(SCIP_STATUS_MAP, scip_status)
