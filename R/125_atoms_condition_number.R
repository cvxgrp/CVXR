...
ConditionNumber <- new_class("ConditionNumber", parent = Atom, package = "CVXR",
  constructor = function(A, id = NULL) {
    ...
    obj <- .fast_new(ConditionNumber, S7_object(),
      id    = as.integer(id),
...
## CVXPY SOURCE: atoms/condition_number.py:46-58 (condition_number._grad).
## CVXPY explicitly raises NotImplementedError. Mirror with list(NULL).
method(.grad, ConditionNumber) <- function(x, values, ...) {
  list(NULL)
}
