...
## CVXPY SOURCE: xpress_conif.py lines 41-50
...
XPRESS_Conic_Solver <- new_class("XPRESS_Conic_Solver", parent = ConicSolver,
  ...
  constructor = function() {
    ...
    .fast_new(XPRESS_Conic_Solver, S7_object(),
...
method(solve_via_data, XPRESS_Conic_Solver) <- function(x, data, warm_start = FALSE,
                                                         verbose = FALSE,
                                                         solver_opts = list(), ...) {
  .require_solver_package(XPRESS_SOLVER)
  ...
  ## CVXPY SOURCE: xpress_conif.py lines 215-235
  ...
  status <- .status_from_map(XPRESS_STATUS_MAP, result$status)
...
method(reduction_invert, XPRESS_Conic_Solver) <- function(x, solution, inverse_data, ...) {
  ...
  status <- .status_from_map(XPRESS_STATUS_MAP, solution$status)
