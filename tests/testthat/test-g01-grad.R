...
test_that("Test gradient for affine_prod", {
  ...
  expect_equivalent(matrix(grad(expr)[[as.character(id(C))]]), arr_val)
  expect_equivalent(matrix(grad(expr)[[as.character(id(B))]]), 
                    rbind(c(1,3,-1,0,0,0), c(-2,4,-3,0,0,0), c(0,0,0,1,3,-1), c(0,0,0,-2,4,-3)))
})
...
test_that("Test gradient for p_norm", {
  ...
  value(x) <- c(-1,0)
  expr <- p_norm(x, 1)
  expect_equivalent(matrix(grad(expr)[[as.character(id(x))]]), matrix(c(-1,0)))

  value(x) <- c(0,10)
  expr <- p_norm(x, 1)
  expect_equivalent(matrix(grad(expr)[[as.character(id(x))]]), matrix(c(0,1)))

  value(x) <- c(-3,4)
  expr <- p_norm(x, 2)
  expect_equivalent(matrix(grad(expr)[[as.character(id(x))]]), matrix(c(-3.0/5,4.0/5)))
  ...
  value(x) <- c(0,0)
  expr <- p_norm(x, 2)
  expect_equivalent(matrix(grad(expr)[[as.character(id(x))]]), matrix(c(0,0)))
  
  value(x) <- c(1,2)
  expr <- p_norm(x, 2, axis = 1)
  val <- diag(2)
  expect_equal(matrix(grad(expr))[[as.character(id(x))]], val)

  value(A) <- rbind(c(2,-2), c(2,2))
  expr <- p_norm(A, 2)
  expect_equivalent(matrix(grad(expr)[[as.character(id(A))]]), matrix(c(0.5,0.5,-0.5,0.5)))

  value(A) <- rbind(c(3,-3), c(4,4))
  expr <- p_norm(A, 2, axis = 2)
  expect_equivalent(matrix(grad(expr)[[as.character(id(A))]]), rbind(c(0.6,0), c(0.8,0), c(0,-0.6), c(0,0.8)))

  value(A) <- rbind(c(3,-4), c(4,3))
  expr <- p_norm(A, 2, axis = 1)
  expect_equivalent(matrix(grad(expr)[[as.character(id(A))]]), rbind(c(0.6,0), c(0,0.8), c(-0.8,0), c(0,0.6)))
  
  value(A) <- rbind(c(0,0), c(10,0))
  expr <- p_norm(A, 2, axis = 1)
  expect_equivalent(matrix(grad(expr)[[as.character(id(A))]]), rbind(c(0,0), c(0,1), c(0,0), c(0,0)))
  
  value(A) <- rbind(c(0,0), c(10,0))
  expr <- p_norm(A, 1, axis = 1)
  expect_equivalent(matrix(grad(expr)[[as.character(id(A))]]), rbind(c(0,0), c(0,1), c(0,0), c(0,0)))
...
test_that("Test gradient for log_sum_exp", {
  ...
  expr <- log_sum_exp(x)
  e <- exp(1)
  expect_equivalent(matrix(grad(expr)[[as.character(id(x))]]), matrix(c(1.0/(1+e), e/(1+e))))

  value(A) <- rbind(c(0,1), c(-1,0))
  expr <- log_sum_exp(A)
  expect_equivalent(matrix(grad(expr)[[as.character(id(A))]]), matrix(c(1.0/(2+e+1.0/e), 1.0/e/(2+e+1.0/e), e/(2+e+1.0/e), 1.0/(2+e+1.0/e))))

  value(A) <- rbind(c(0,1), c(-1,0))
  expr <- log_sum_exp(A, axis = 2)
  expect_equivalent(matrix(grad(expr)[[as.character(id(A))]]), cbind(c(1.0/(1+1.0/e), 1.0/e/(1+1.0/e), 0, 0), c(0, 0, e/(1+e), 1.0/(1+e))))
})
...
test_that("Test gradient for geo_mean", {
  ...
  value(x) <- c(1,2)
  expr <- geo_mean(x)
  expect_equivalent(matrix(grad(expr)[[as.character(id(x))]]), matrix(c(sqrt(2)/2, 1.0/2/sqrt(2))))
...
test_that("Test gradient for lambda_max", {
  ...
  expr <- lambda_max(A)
  expect_equivalent(matrix(grad(expr)[[as.character(id(A))]]), matrix(c(1,0,0,0)))

  value(A) <- cbind(c(1,0), c(0,2))
  expr <- lambda_max(A)
  expect_equivalent(matrix(grad(expr)[[as.character(id(A))]]), matrix(c(0,0,0,1)))
...
test_that("Test gradient for matrix_frac", {
  ...
  expect_equivalent(matrix(grad(expr)[[as.character(id(A))]]), matrix(c(2,0,0,2)))
  expect_equivalent(matrix(grad(expr)[[as.character(id(B))]]), matrix(c(-1,0,0,-1)))
  ...
  expect_equivalent(matrix(grad(expr)[[as.character(id(x))]]), matrix(c(4,6)))
  expect_equivalent(matrix(grad(expr)[[as.character(id(A))]]), matrix(c(-4,-6,-6,-9)))
})
...
test_that("Test gradient for norm_nuc", {
  ...
  expr <- norm_nuc(A)
  expect_equivalent(matrix(grad(expr)[[as.character(id(A))]]), matrix(c(1,0,0,1)))
})
...
test_that("Test gradient for log_det", {
  ...
  value(A) <- 2*diag(2)
  expr <- log_det(A)
  expect_equivalent(matrix(grad(expr)[[as.character(id(A))]]), 1.0/2*diag(2))
  ...
  K <- Variable(8,8)
  value(K) <- diag(8)
  expr <- log_det(K[c(1,2), c(1,2)])
  val <- matrix(0, nrow = 8, ncol = 8)
  val[c(1,2), c(1,2)] <- 1
  expect_equal(grad(expr)[[as.character(id(K))]], val)
})
...
test_that("Test gradient for quad_over_lin", {
  ...
  expect_equivalent(matrix(grad(expr)[[as.character(id(x))]]), matrix(c(1,2)))
  expect_equal(grad(expr)[[as.character(id(a))]], -1.25)
  ...
  expect_equivalent(matrix(grad(expr)[[as.character(id(A))]]), matrix(c(1,0,0,1)))
  expect_equal(grad(expr)[[as.character(id(a))]], -0.5)
  ...
  expect_equivalent(matrix(grad(expr)[[as.character(id(x))]]), matrix(c(1,2)))
  expect_equivalent(matrix(grad(expr)[[as.character(id(y))]]), matrix(c(1,2)))
  expect_equal(grad(expr)[[as.character(id(a))]], -2.5)
})

test_that("test gradient for quad_form", {
  # Issue 1260.
  n <- 10
  set.seed(1)
  ...
  x <- Variable(n)
  prob <- Problem(Maximize(t(q) %*% x - (1/2)*quad_form(x, P)),
                  list(norm1(x) <= 1.0, quad_form(x, P) <= 10, abs(x) <= 0.01))
  result <- solve(prob, solver = "SCS")
  
  # Access quad_form@expr@grad without error.
  prob@constraints[[2]]@expr@grad
...
test_that("Test gradient for max_entries", {
  ...
  expr <- max_entries(A)
  expect_equivalent(matrix(grad(expr)[[as.character(id(A))]]), matrix(c(0,1,0,0)))

  value(A) <- rbind(c(1,2), c(4,3))
  expr <- max_entries(A, axis = 2)
  expect_equivalent(matrix(grad(expr)[[as.character(id(A))]]), rbind(c(0,0), c(1,0), c(0,0), c(0,1)))

  value(A) <- rbind(c(1,2), c(4,3))
  expr <- max_entries(A, axis = 1)
  expect_equivalent(matrix(grad(expr)[[as.character(id(A))]]), rbind(c(0,0), c(0,1), c(1,0), c(0,0)))
})
...
test_that("Test sum_largest", {
  ...
  expect_equivalent(matrix(grad(expr)[[as.character(id(A))]]), matrix(c(1,0,1,0)))

  value(A) <- cbind(c(1,2), c(3,0.5))
  expr <- sum_largest(A, 2)
  expect_equivalent(matrix(grad(expr)[[as.character(id(A))]]), matrix(c(0,1,1,0)))
})

test_that("test dotsort", {
  skip_on_cran()
  value(A) <- rbind(c(4,3), c(2,1))
  expr <- dotsort(A, c(0.1, -2))
  expect_equivalent(matrix(grad(expr)[[as.character(id(A))]]), matrix(c(0.1,0,0,-2)))
  
  value(A) <- rbind(c(1,2), c(3,0.5))
  expr <- dotsort(A, c(0.1, -2))
  expect_equivalent(matrix(grad(expr)[[as.character(id(A))]]), matrix(c(0,0.1,0,-2)))
  
  # sum_largest tests.
  value(A) <- rbind(c(4,3), c(2,1))
  expr <- dotsort(A, c(1,1))
  expect_equivalent(matrix(grad(expr)[[as.character(id(A))]]), matrix(c(1,0,1,0)))
  value(A) <- rbind(c(1,2), c(3,0.5))
  expr <- dotsort(A, c(1,1))
  expect_equivalent(matrix(grad(expr)[[as.character(id(A))]]), matrix(c(0,1,1,0)))
  ...
  value(A) <- rbind(c(4,3), c(2,1))
  expr <- -dotsort(A, c(-1,-1))
  expect_equivalent(matrix(grad(expr)[[as.character(id(A))]]), matrix(c(0,1,0,1)))
  value(A) <- rbind(c(1,2), c(3,0.5))
  expr <- -dotsort(A, c(-1,-1))
  expect_equivalent(matrix(grad(expr)[[as.character(id(A))]]), matrix(c(1,0,0,1)))
})
...
test_that("Test linearize method", {
  ...
  expect_equivalent(matrix(value(lin_expr)), value(expr))
  ...
  expect_equivalent(matrix(value(lin_expr)), value(manual))
...
test_that("Test gradient for log", {
  ...
  value(a) <- 2
  expr <- log(a)
  expect_equal(grad(expr)[[as.character(id(a))]], 1.0/2)

  value(a) <- 3
  expr <- log(a)
  expect_equal(grad(expr)[[as.character(id(a))]], 1.0/3)
...
test_that("Test domain for log1p", {
  ...
  value(a) <- 3
  expr <- log1p(a)
  expect_equal(grad(expr)[[as.character(id(a))]], 1.0/4)
...
test_that("Test domain for entr", {
  ...
  value(a) <- 2
  expr <- entr(a)
  expect_equal(grad(expr)[[as.character(id(a))]], -log(2)-1)

  value(a) <- 3
  expr <- entr(a)
  expect_equal(grad(expr)[[as.character(id(a))]], -(log(3)+1))
...
test_that("Test domain for exp", {
  ...
  value(a) <- 2
  expr <- exp(a)
  expect_equal(grad(expr)[[as.character(id(a))]], exp(2))

  value(a) <- 3
  expr <- exp(a)
  expect_equal(grad(expr)[[as.character(id(a))]], exp(3))

  value(a) <- -1
  expr <- exp(a)
  expect_equal(grad(expr)[[as.character(id(a))]], exp(-1))
...
test_that("Test domain for logistic", {
  ...
  value(a) <- 2
  expr <- logistic(a)
  expect_equal(grad(expr)[[as.character(id(a))]], exp(2)/(1+exp(2)))

  value(a) <- 3
  expr <- logistic(a)
  expect_equal(grad(expr)[[as.character(id(a))]], exp(3)/(1+exp(3)))

  value(a) <- -1
  expr <- logistic(a)
  expect_equal(grad(expr)[[as.character(id(a))]], exp(-1)/(1+exp(-1)))
...
test_that("Test domain for huber", {
  ...
  value(a) <- 2
  expr <- CVXR::huber(a)
  expect_equal(grad(expr)[[as.character(id(a))]], 2)

  value(a) <- 3
  expr <- CVXR::huber(a, M = 2)
  expect_equal(grad(expr)[[as.character(id(a))]], 4)
...
test_that("Test domain for kl_div", {
  ...
  value(b) <- 4
  expr <- kl_div(a, b)
  expect_equal(grad(expr)[[as.character(id(a))]], log(2/4))
  expect_equal(grad(expr)[[as.character(id(b))]], 1-(2/4))
...
test_that("Test domain for max_elemwise", {
  ...
  expect_equal(grad(expr)[[as.character(id(a))]], 0)
  expect_equal(grad(expr)[[as.character(id(b))]], 1)

  value(a) <- 3
  value(b) <- 0
  expr <- max_elemwise(a, b)
  expect_equal(grad(expr)[[as.character(id(a))]], 1)
  expect_equal(grad(expr)[[as.character(id(b))]], 0)
...
test_that("Test domain for power", {
  ...
  value(a) <- 2
  expr <- sqrt(a)
  expect_equal(grad(expr)[[as.character(id(a))]], 0.5/sqrt(2))

  value(a) <- 3
  expr <- sqrt(a)
  expect_equal(grad(expr)[[as.character(id(a))]], 0.5/sqrt(3))
  ...
  value(x) <- c(3,4)
  expr <- x^3
  expect_equivalent(matrix(grad(expr)[[as.character(id(x))]]), rbind(c(27,0), c(0,48)))
...
test_that("Test grad for affine atoms", {
  ...
  expect_equivalent(matrix(grad(expr)[[as.character(id(z))]]), val)
