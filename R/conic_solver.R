...
ConicSolver <- setClass("ConicSolver", representation(dims = "character", # The key that maps to ConeDims in the data returned by perform().
                        ...
                        prototype(dims = "dims", exp_cone_order = integer(0)), contains = "ReductionSolver")

# Every conic solver must support Zero and NonNeg constraints.
setMethod("supported_constraints", "ConicSolver", function(solver) { c("ZeroConstraint", "NonNegConstraint") })
...
setMethod("supports_quad_obj", "ConicSolver", function(solver) { FALSE })
