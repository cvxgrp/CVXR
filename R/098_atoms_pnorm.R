...
# -- .column_grad: subgradient of an Lp norm ----------------------
## CVXPY SOURCE: atoms/pnorm.py:225-262 (pnorm._column_grad).
## d/dx ||x||_p = sign(x_i) * |x_i|^(p-1) / ||x||_p^(p-1)  (for p > 1).
## For p < 1 (concave on positive orthant): nominator = x^(p-1).
## Domain check (p < 1, any nonpositive entry) returns NULL.
## At zero denominator: subgrad is 0 if p > 1, NULL otherwise.
method(.column_grad, Pnorm) <- function(x, value, ...) {
  v <- as.numeric(value)
  p <- as.numeric(x@p)        # Coerce in case p is bigq from PnormApprox.
  ## Domain: p < 1 requires positive arg.
  if (p < 1 && any(v <= 0)) return(NULL)
  exp <- p - 1
  denom_norm <- sum(abs(v)^p)^(1/p)
  denom <- denom_norm^exp
  if (denom == 0) {
    if (p > 1) {
      return(numeric(length(v)))   # all zeros
    } else {
      return(NULL)
    }
  }
  if (p > 1) {
    nom <- sign(v) * abs(v)^exp
  } else {
    nom <- v^exp
  }
  nom / denom
...
PnormApprox <- new_class("PnormApprox", parent = Pnorm, package = "CVXR",
  properties = list(
    ...
    original_p   = new_property(class = class_numeric)  # CVXPY pnorm.py:134
  ),
  constructor = function(x, p = 2, axis = NULL, keepdims = FALSE,
                         max_denom = 1024L, id = NULL) {
    if (FALSE) new_object(S7_object())  ## S7 static-check guard
    ...
    obj <- .fast_new(PnormApprox, S7_object(),
      id           = as.integer(id),
      .cache       = new.env(parent = emptyenv()),
      args         = list(x),
      shape        = shape,
