...
# -- smoothness (CVXPY prod.py:73-75): prod is smooth, so a prod over
# nonneg arguments is both linearizable-convex and linearizable-concave,
# making prod problems DNLP (solvable through the NLP path).
...
# -- .column_grad: prod(value) / value ----------------------------
## CVXPY SOURCE: atoms/prod.py:111-122 (Prod._column_grad).
## d/dx_i prod(x) = prod(x) / x_i, the product over all entries except i.
## NaN risk on zero entries; CVXPY relies on numpy broadcasting which
## yields inf/nan -- behaviour is preserved since the gradient is
## undefined at zero anyway (distinct from "not implemented").
method(.column_grad, Prod) <- function(x, value, ...) {
  v <- as.numeric(value)
  prod(v) / v
}
