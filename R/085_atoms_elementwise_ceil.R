...
## numeric: floor with tolerance rounding
## CVXPY SOURCE: ceil.py:109-112 (CVXPY 1.9.2) -- `np.floor(np.around(...))`.
## Ceil already rounded first; floor did not, so a value a hair under an
## integer (3 - 1e-10, ordinary solver noise) floored to the integer BELOW.
## Both R's round() and numpy's around() break ties to even, so 2.5 -> 2.
method(numeric_value, Floor) <- function(x, values, ...) {
  decimals <- as.integer(abs(log10(ATOM_EVAL_TOL)))
  floor(round(values[[1L]], digits = decimals))
}
