...
## CVXPY v1.9.0 fix: #3197 -- carry an (infeasibility-certificate) dual_vars
## map through failure solutions so infeasible/unbounded problems can expose
## their dual rays. Defaults to empty, so callers that omit it are unaffected.

failure_solution <- function(status, attr = list(), dual_vars = list()) {
