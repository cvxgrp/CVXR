...
VStack <- new_class("VStack", parent = AffAtom, package = "CVXR",
  constructor = function(..., id = NULL) {
    ...
    cols <- .shape(all_args[[1L]])[2L]
    total_rows <- 0L
    for (arg in all_args) {
      if (.shape(arg)[2L] != cols) {
        cli_abort("All arguments to {.fn vstack} must have the same number of columns.")
      }
      total_rows <- total_rows + .shape(arg)[1L]
    }
    shape <- c(total_rows, cols)

    obj <- .fast_new(VStack, S7_object(),
      id    = as.integer(id),
...
method(shape_from_args, VStack) <- function(x) {
  cols <- .arg_shape(x)[2L]
  total_rows <- sum(vapply(.args(x), function(a) .shape(a)[1L], integer(1)))
  c(total_rows, cols)
}
