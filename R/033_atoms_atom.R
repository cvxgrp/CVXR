...
method(expr_name, Atom) <- function(x) {
  ...
  arg_strs <- vapply(.args(x), expr_name, character(1))
...
method(format_labeled, Atom) <- function(x) {
  ...
  data <- get_data(x)
  data_str <- if (is.null(data)) character(0) else vapply(data, as.character, character(1))
  arg_strs <- vapply(.args(x), format_labeled, character(1))
  sprintf("%s(%s)", class(x)[[1L]], paste(c(arg_strs, data_str), collapse = ", "))
}
...
.non_const_idx <- function(x) {
  ...
  for (i in seq_along(.args(x))) {
    if (!is_constant(.args(x)[[i]])) idx <- c(idx, i)
  }
  idx
}
...
.is_real_fn <- function(x) {
  ...
  expr_is_scalar(x) &&
    length(non_const) == 1L &&
    expr_is_scalar(.args(x)[[non_const[1L]]])
}
...
.atom_is_quasiconvex <- function(x) {
  ...
  if (.is_real_fn(x) && is_incr(x, non_const[1L])) {
    return(is_quasiconvex(.args(x)[[non_const[1L]]]))
  }
  if (.is_real_fn(x) && is_decr(x, non_const[1L])) {
    return(is_quasiconcave(.args(x)[[non_const[1L]]]))
  }
...
method(canonicalize, Atom) <- function(x) {
  ...
  arg_objs <- vector("list", length(.args(x)))
  constraints <- list()
  for (i in seq_along(.args(x))) {
    cf <- canonical_form(.args(x)[[i]])
    arg_objs[[i]] <- cf[[1L]]
    constraints <- c(constraints, cf[[2L]])
  }
  ## Graph implementation
  data <- get_data(x)
  gi <- graph_implementation(x, arg_objs, .shape(x), data)
...
.atom_value_impl <- function(x) {
  if (0L %in% .shape(x)) return(numeric(0))
  arg_values <- vector("list", length(.args(x)))
  for (i in seq_along(.args(x))) {
    arg_val <- value(.args(x)[[i]])
...
method(domain, Atom) <- function(x) {
  ...
  arg_domains <- unlist(lapply(.args(x), domain), recursive = FALSE)
