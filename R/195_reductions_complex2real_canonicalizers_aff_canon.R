  ...
  if (all(vapply(imag_args, is.null, logical(1L)))) {
    ...
    for (idx in seq_along(real_args)) {
      if (is.null(real_args[[idx]])) {
        real_args[[idx]] <- Constant(matrix(0, .shape(imag_args[[idx]])[1L],
                                            .shape(imag_args[[idx]])[2L]))
      } else if (is.null(imag_args[[idx]])) {
        imag_args[[idx]] <- Constant(matrix(0, .shape(real_args[[idx]])[1L],
                                            .shape(real_args[[idx]])[2L]))
      }
    }
