...
##  - DNLP objective predicates mirror objective.py:159-163 and 236-240:
##    Minimize requires a linearizable-convex target; Maximize requires a
##    linearizable-concave target.
...
## is_dnlp: the objective expression must be linearizable-convex
## CVXPY SOURCE: objective.py lines 159-163
method(is_dnlp, Minimize) <- function(x) is_linearizable_convex(x@args[[1L]])
...
## is_dnlp: the objective expression must be linearizable-concave
## CVXPY SOURCE: objective.py lines 236-240
method(is_dnlp, Maximize) <- function(x) is_linearizable_concave(x@args[[1L]])
...
.negate_objective <- function(obj) {
  ...
  expr <- obj@args[[1L]]
  if (.s7_is(obj, Minimize)) Maximize(-expr) else Minimize(-expr)
}
...
.add_objectives <- function(o1, o2) {
  if (!.s7_is(o2, Objective))
    cli_abort("Cannot add {.cls {class(o2)[[1L]]}} to an objective.")
  e1 <- o1@args[[1L]]
  e2 <- o2@args[[1L]]
  if (.s7_is(o1, Minimize) && .s7_is(o2, Minimize)) {
    Minimize(e1 + e2)
  } else if (.s7_is(o1, Maximize) && .s7_is(o2, Maximize)) {
    Maximize(e1 + e2)
  } else {
...
.mul_objective <- function(obj, scalar) {
  ...
  if ((.s7_is(obj, Maximize)) == (scalar < 0)) {
