...
complex2real_accepts <- function(problem) {
  ## `parameters(problem)` is a load-bearing term, not a redundant one:
  ## `problem.constants()` (problem.py:484-499) collects only `Constant` leaves,
  ## and a `Parameter` is not a `Constant`, so without it a problem whose ONLY
  ## complex leaf is a Parameter reports FALSE. CVXR omitted it, and the
  ## consequence was not a missing reduction but an error: `has_complex_params`
  ## (solving_chain.R:650-652) still prepends `EvalParams`, which turns the
  ## complex Parameter into a complex Constant with no Complex2Real downstream,
  ## so the solve died with "Inequality constraints cannot be complex."
  ## Measured on `Minimize(abs(p %*% x)), x >= 1` with p a complex Parameter:
  ## CVXPY 1.9.2 accepts TRUE and solves to 4.123105626514879.
...
method(reduction_apply, Complex2Real) <- function(x, problem, ...) {
  ...
  for (v in variables(problem)) {
    if (is_complex(v)) {
      assign(as.character(.id(v)), next_expr_id(), envir = real2imag)
    }
  }
  for (p in parameters(problem)) {
    if (is_complex(p)) {
      assign(as.character(.id(p)), next_expr_id(), envir = real2imag)
    }
  }
  for (con in problem@constraints) {
    if (is_complex(con)) {
      assign(as.character(.id(con)), next_expr_id(), envir = real2imag)
    }
  }
  ...
  ## Original complex leaf objects keyed by id -- the dict-diff hooks
  ## (var_backward/var_forward/param_*) need is_complex/is_hermitian/shape to
  ## split or recombine the real/imag parts (#3147 part A). reduction_invert
  ## uses inverse_data$id2var instead.
  id_to_var <- new.env(hash = TRUE, parent = emptyenv())
  for (v in variables(problem)) {
    if (is_complex(v)) assign(as.character(.id(v)), v, envir = id_to_var)
  }
  x@.cache$id_to_var <- id_to_var
  id_to_param <- new.env(hash = TRUE, parent = emptyenv())
  for (p in parameters(problem)) {
    if (is_complex(p)) assign(as.character(.id(p)), p, envir = id_to_param)
  }
  ...
  for (v in variables(problem)) {
    assign(as.character(.id(v)), v, envir = inverse_data$id2var)
  }
  for (con in problem@constraints) {
    assign(as.character(.id(con)), con, envir = inverse_data$id2cons)
  }
  ...
  if (solution@status %in% SOLUTION_PRESENT) {
    ...
    if (length(solution@dual_vars) > 0L) {
      for (cid in ls(id2cons)) {
        ...
        if (is_real(con)) {
        ...
        } else if (.s7_is(con, PSD)) {
          ## CVXPY SOURCE: complex2real.py:340-349 (CVXPY 1.9.2).
          ## A Hermitian X >> 0 is solved through the real dilation
          ##     Y = [[re(X), -im(X)], [im(X), re(X)]] >> 0.
          ## If D is the dual for the dilated constraint, the dual for the
          ## original is 2*(D[1:n, 1:n] + 1i*D[(n+1):2n, 1:n]).  The factor
          ## of 2 is NOT cosmetic: the dilation doubles the inner product,
          ##     <D, Y> = 2*Re(<D[1:n,1:n] + 1i*D[(n+1):2n,1:n], X>),
          ## so without it the recovered dual is exactly HALF its true value
          ## and violates both KKT and strong duality.  Measured before this
          ## fix, with C = [[2, 1-1i], [1+1i, 3]] and X >> A: CVXR returned
          ## [[1, 0.5+0.5i], [0.5-0.5i, 1.5]] where KKT gives Y == C.
          n <- .arg_shape(con)[1L]
          ...
          if (is.null(dual)) {
            ...
            dvars[[cid]] <- 2 * (dual[1L:n, 1L:n] +
                                   1i * dual[(n + 1L):nn, 1L:n])
          }
        } else if (.s7_is(con, SOC)) {
...
.c2r_canonicalize_tree <- function(expr, real2imag, leaf_map) {
  ...
  for (i in seq_len(n_args)) {
    result <- .c2r_canonicalize_tree(.args(expr)[[i]], real2imag, leaf_map)
...
.c2r_canonicalize_expr <- function(expr, real_args, imag_args, real2imag, leaf_map) {
  ...
  if (has_c2r_canon(expr) && length(.args(expr)) == 0L) {
    key <- as.character(.id(expr))
...
## SPLIT a complex leaf's value into a real part (kept at the original id) and an
## imaginary part (at the fresh real2imag id). Used by var_backward (gradients,
## outer->inner) and param_forward (deltas, outer->inner).
## CVXPY SOURCE: complex2real.py:121-141 / 211-242. Keep-dims throughout
## (ADR D_19.5 addendum 2): every value is a shaped array of the leaf's shape, or
## the compact strict-upper-triangle (k x 1) for the Hermitian imaginary part.
.c2r_split <- function(dict, leaf_env, real2imag) {
  for (id in ls(leaf_env, all.names = TRUE)) {
    if (is.null(dict[[id]])) next
    leaf <- get(id, envir = leaf_env, inherits = FALSE)
    imag_id <- if (exists(id, envir = real2imag, inherits = FALSE))
      as.character(get(id, envir = real2imag, inherits = FALSE)) else NULL
    shp <- .shape(leaf)
    z <- as.complex(dict[[id]])
    dict[[id]] <- array(Re(z), dim = shp)            # real part keeps the id
    if (!is.null(imag_id)) {
      im <- Im(z)
      if (is_hermitian(leaf)) {
        n <- shp[1L]; mat <- matrix(im, n, n)
        tri <- mat[upper.tri(mat, diag = FALSE)]
        dict[[imag_id]] <- array(tri, dim = c(length(tri), 1L))
      } else {
        dict[[imag_id]] <- array(im, dim = shp)
      }
    }
  }
  dict
...
## COMBINE a real part (original id) and imaginary part (real2imag id) back into a
## complex value at the original id, popping the imaginary id. Used by var_forward
## (deltas, inner->outer) and param_backward (gradients, inner->outer).
## CVXPY SOURCE: complex2real.py:143-170 / 172-209.
.c2r_combine <- function(dict, leaf_env, real2imag) {
  for (id in ls(leaf_env, all.names = TRUE)) {
    leaf <- get(id, envir = leaf_env, inherits = FALSE)
    imag_id <- if (exists(id, envir = real2imag, inherits = FALSE))
      as.character(get(id, envir = real2imag, inherits = FALSE)) else NULL
    shp <- .shape(leaf)
    re <- if (!is.null(dict[[id]])) array(as.numeric(dict[[id]]), dim = shp)
          else array(0, dim = shp)
    im_part <- array(0, dim = shp)
    if (!is.null(imag_id) && !is.null(dict[[imag_id]])) {
      raw_im <- as.numeric(dict[[imag_id]])
      if (is_hermitian(leaf)) {
        n <- shp[1L]; full <- matrix(0, n, n)
        full[upper.tri(full, diag = FALSE)] <- raw_im
        full <- full - t(full)
        im_part <- full
      } else {
        im_part <- array(raw_im, dim = shp)
      }
      dict[[imag_id]] <- NULL                          # pop the imag id
    }
    dim(im_part) <- shp                                # keep-dims guard
    dict[[id]] <- re + 1i * im_part                    # array + array, same dim
  }
  dict
}

## Dict-in/dict-out chain rule (#3147 part A). Directional asymmetry: backward
## SPLITS variables but COMBINES parameters; forward is the reverse.
method(var_backward, Complex2Real) <- function(x, del_vars) {
  if (is.null(x@.cache$real2imag)) return(del_vars)
  .c2r_split(del_vars, x@.cache$id_to_var, x@.cache$real2imag)
}
method(var_forward, Complex2Real) <- function(x, dvars) {
  if (is.null(x@.cache$real2imag)) return(dvars)
  .c2r_combine(dvars, x@.cache$id_to_var, x@.cache$real2imag)
}
method(param_backward, Complex2Real) <- function(x, dparams) {
  if (is.null(x@.cache$real2imag)) return(dparams)
  .c2r_combine(dparams, x@.cache$id_to_param, x@.cache$real2imag)
}
method(param_forward, Complex2Real) <- function(x, param_deltas) {
  if (is.null(x@.cache$real2imag)) return(param_deltas)
  .c2r_split(param_deltas, x@.cache$id_to_param, x@.cache$real2imag)
}
