...
DistRatio <- new_class("DistRatio", parent = Atom, package = "CVXR",
  ...
  constructor = function(x, a, b, id = NULL) {
    ...
    obj <- .fast_new(DistRatio, S7_object(),
      id    = as.integer(id),
...
# -- .grad: not implemented (mirrors CVXPY's [None, None, None]) -
## CVXPY SOURCE: atoms/dist_ratio.py:84-85 (dist_ratio._grad).
## DistRatio takes 3 args (x, a, b). CVXPY returns [None, None, None];
## mirror with list(NULL, NULL, NULL).
method(.grad, DistRatio) <- function(x, values, ...) {
  list(NULL, NULL, NULL)
}
