...
GUROBI_STATUS_MAP <- list(
  ...
  ## CVXPY SOURCE: gurobi_conif.py:57 and gurobi_qpif.py:47 -- `13: s.USER_LIMIT,
  ## # SUBOPTIMAL`. This was OPTIMAL_INACCURATE, the ONE key of sixteen that
  ## disagreed with upstream, and the disagreement was self-concealing: the
  ## demotion rule below (reduction_invert, gurobi_conif.py:301-302) reads
  ## `status == USER_LIMIT && is.null(solution$x)` to turn a limit-stop with no
  ## incumbent into INFEASIBLE_INACCURATE, and SUBOPTIMAL could never reach it.
  ## So a Gurobi run that stopped suboptimally with NO solution was reported
  ## OPTIMAL_INACCURATE -- a claim that a solution exists and is nearly optimal.
  "SUBOPTIMAL"      = USER_LIMIT,
...
Gurobi_Conic_Solver <- new_class("Gurobi_Conic_Solver", parent = ConicSolver, package = "CVXR",
  constructor = function() {
    ...
    .fast_new(Gurobi_Conic_Solver, S7_object(),
