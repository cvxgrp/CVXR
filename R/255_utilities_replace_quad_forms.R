...
replace_quad_forms <- function(expr, quad_forms = list()) {
  ...
  for (idx in seq_along(.args(expr))) {
    arg <- .args(expr)[[idx]]
    if (.s7_is(arg, SymbolicQuadForm) || .s7_is(arg, QuadForm)) {
      ## Replace with dummy Variable
      placeholder <- Variable(.shape(arg), var_id = .id(arg))
...
restore_quad_forms <- function(expr, quad_forms) {
  ...
  for (idx in seq_along(.args(expr))) {
    ...
    if (.s7_is(arg, Variable) && !is.null(quad_forms[[arg_id]])) {
