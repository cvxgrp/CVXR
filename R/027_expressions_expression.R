...
##  - DNLP predicates mirror expression.py:349-405: `is_smooth`,
##    `is_dnlp`, and the linearizable-convex / linearizable-concave hooks.
##  - CVXPY makes the linearizable hooks abstract on Expression; CVXR uses
##    FALSE defaults, matching the existing `is_convex` / `is_concave` default
##    style and relying on Leaf, Atom, and transforms for concrete behavior.
...
method(is_constant, Expression) <- function(x) {
  ...
  result <- (0L %in% .shape(x)) || .all_args(x, is_constant)
...
method(is_dpp, Expression) <- function(x, context = "dcp") {
  base_ok <- if (identical(tolower(context), "dgp")) with_dpp_scope(is_dgp(x))
             else with_dpp_scope(is_dcp(x))
  if (!base_ok) return(FALSE)
  for (arg in .args(x)) {
    if (!is_dpp(arg, context)) return(FALSE)
  }
  TRUE
}
...
# -- DNLP curvature (CVXPY 1.9.0) ----------------------------------
## is_linearizable_convex / is_linearizable_concave -- abstract on Expression
## (CVXPY raises NotImplementedError); concrete on Leaf / Atom / Indicator /
## PartialProblem. Default FALSE mirrors the is_convex / is_concave defaults.
## CVXPY SOURCE: expression.py lines 369-379
method(is_linearizable_convex, Expression) <- function(x) FALSE
method(is_linearizable_concave, Expression) <- function(x) FALSE

## is_smooth(): constant OR (linearizable-convex AND linearizable-concave)
## CVXPY SOURCE: expression.py lines 349-355 (@perf.compute_once)
method(is_smooth, Expression) <- function(x) {
  key <- .dpp_key("is_smooth")
  cached <- cache_get(x, key)
  if (!cache_miss(cached)) return(cached)
  result <- is_constant(x) ||
    (is_linearizable_convex(x) && is_linearizable_concave(x))
  cache_set(x, key, result)
  result
}

## is_dnlp(): linearizable-convex OR linearizable-concave
## CVXPY SOURCE: expression.py lines 401-405 (no compute_once)
method(is_dnlp, Expression) <- function(x) {
  is_linearizable_convex(x) || is_linearizable_concave(x)
}
...
is_log_log_constant <- function(x) {
  ...
  if (.s7_is(x, Constant) || .s7_is(x, Parameter)) {
    return(is_pos(x))
  }
...
# -- label / set_label / format_labeled --------------------------------
## CVXPY SOURCE: expression.py lines 117-244
##
## Storage: each Expression already carries a `.cache` env (mutable
## reference semantics).  CVXPY uses `self._label = None`; we use
## `x@.cache$.label` and treat absent / NULL as "no label set".

method(label, Expression) <- function(x) {
  if (exists(".label", envir = x@.cache, inherits = FALSE)) {
    get(".label", envir = x@.cache, inherits = FALSE)
  } else NULL
}

## CVXPY SOURCE: expression.py lines 189-205 (label setter + deleter)
method(`label<-`, Expression) <- function(x, value) {
  if (is.null(value)) {
    if (exists(".label", envir = x@.cache, inherits = FALSE)) {
      rm(list = ".label", envir = x@.cache)
    }
  } else {
    assign(".label", as.character(value)[[1L]], envir = x@.cache)
  }
  invisible(x)
}

## CVXPY SOURCE: expression.py lines 207-228 (set_label, returns self).
method(set_label, Expression) <- function(x, value) {
  label(x) <- value
  invisible(x)
}

## CVXPY SOURCE: expression.py lines 230-244 (format_labeled base).
## If a label is set, return it; otherwise default to expr_name().
## Atom subclasses with a custom expr_name override format_labeled to
## recurse via format_labeled() on children (see atom.R and per-atom files).
method(format_labeled, Expression) <- function(x) {
  lbl <- label(x)
...
## CVXPY v1.9.0 fix: #3180 _pow_const_base (expression.py:57-72) + __rpow__
## (expression.py:688-705). Helper for b^x where b is a positive constant and
## x is a CVXPY Expression. Validates that base is constant and positive, then
## returns exp(x * log(b)) via the identity b^x = exp(x * log(b)).
## In CVXR, `^` (Ops group, both directions) and the `power()` factory route
## the const-base / variable-exponent case here.
.pow_const_base <- function(base, exponent) {
  base <- as_expr(base)
  if (!is_constant(base)) {
    cli_abort("The base of {.code b^x} must be a constant when the exponent is a variable.")
  }
  if (!is_pos(base)) {
    cli_abort("The base of {.code b^x} must be positive since we use {.code b^x = exp(x * log(b))}.")
  }
