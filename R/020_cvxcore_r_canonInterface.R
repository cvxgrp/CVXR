...
##
## Returns the rows of the parameter -> problem-data tensor that ANY parameter
## can drive: i.e. the entries of `A` that are a function of a parameter, as
## opposed to structurally absent. `apply_parameters(keep_zeros = TRUE)` then
## re-inserts those entries as EXPLICIT ZEROS, so the sparsity pattern of `A`
## describes which entries a parameter *can* affect rather than which ones
## happen to be nonzero at the current parameter value. Only DIFFCP asks for
## this, and it is the difference between a correct derivative and a zero one.
##
## Two details are load-bearing and both are upstream's:
##   - the row range stops at `A_nrows * var_length`, excluding the trailing
##     block that holds `b` (the tensor is the column-major flattening of the
##     (A | b) matrix, so `b` is the last column and thus the last block);
##   - the CONSTANT column of the parameter axis is dropped, so a row that only
##     a constant feeds is NOT counted. CVXR builds that column last
##     (reductions/reduction.R:43,56 -- "parameter columns are first,
##     CONSTANT_ID is last"), matching upstream's `[:, :-1]`, but the column is
##     read from `param_id_map` here rather than assumed.
##
## Returns 1-BASED flat (column-major) row indices into the tensor.
A_mapping_nonzero_rows <- function(problem_data_tensor, var_length,
                                   const_col = NULL) {
  n_cols <- var_length + 1L
  if (nrow(problem_data_tensor) == 0L || var_length == 0L) return(integer(0))
  a_nrows <- nrow(problem_data_tensor) %/% n_cols
  a_block <- a_nrows * var_length
  if (a_block == 0L) return(integer(0))

  ## Drop the constant column: `const_col` is 0-based, as stored in param_id_map.
  keep_cols <- seq_len(ncol(problem_data_tensor))
  if (!is.null(const_col)) keep_cols <- keep_cols[keep_cols != (const_col + 1L)]
  if (length(keep_cols) == 0L) return(integer(0))

  sub <- problem_data_tensor[seq_len(a_block), keep_cols, drop = FALSE]
  sub <- methods::as(methods::as(sub, "CsparseMatrix"), "TsparseMatrix")
  ## "don't count explicit zeros" -- upstream's comment, and it matters: the
  ## tensor itself may carry structural zeros from canonicalization.
  nz <- sub@x != 0
  if (!any(nz)) return(integer(0))
  sort(unique(sub@i[nz])) + 1L
}
...
format_matrix <- function(matrix, format='dense') {
    ...
    ## Convert gmp big rational/integer to double.
    ## No availability guard: gmp is a mandatory Import (DESCRIPTION,
    ## `gmp (>= 0.7)`), so `requireNamespace("gmp")` was always TRUE -- a dead
    ## probe on a hot path (2005 calls per `large_lp` solve). `is.bigq`/`is.bigz`
    ## are class tests, so `inherits()` answers the same question without
    ## reaching into the namespace at all.
    if (inherits(matrix, c("bigq", "bigz"))) {
