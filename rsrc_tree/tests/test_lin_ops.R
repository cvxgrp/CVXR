...
create_const <- CVXR:::lu.create_const
create_eq <- CVXR:::lu.create_eq
create_leq <- CVXR:::lu.create_leq
create_param <- CVXR:::lu.create_param
create_var <- CVXR:::lu.create_var
get_expr_vars <- CVXR:::lu.get_expr_vars

lu.neg_expr <- CVXR:::lu.neg_expr
lu.sum_expr <- CVXR:::lu.sum_expr
lu.sum_entries <- CVXR:::lu.sum_entries

test_that("Test creating a variable", {
  skip_on_cran()
  var <- lu.create_var(c(5, 4), var_id = 1)
...
test_that("Test creating a constant", {
  ...
  dim <- c(5, 4)
  mat <- lu.create_const(matrix(1, nrow = dim[1], ncol = dim[2]), dim)
  expect_equal(mat$dim, dim)
  expect_equal(length(mat$args), 0)
  ...
  dim <- c(5, 5)
  mat <- lu.create_const(Matrix(diag(5), sparse = TRUE), dim, sparse = TRUE)
  expect_equal(mat$dim, dim)
  expect_equal(length(mat$args), 0)
...
test_that("Test getting vars from an expression", {
  ...
  x <- lu.create_var(dim)
  y <- lu.create_var(dim)
  A <- lu.create_const(matrix(1, nrow = dim[1], ncol = dim[2]), dim)

  # Expanding dict.
  add_expr <- lu.sum_expr(list(x, y, A))
...
test_that("Test creating an equality constraint", {
  ...
  x <- lu.create_var(dim)
  y <- lu.create_var(dim)
  lh_expr <- lu.sum_expr(list(x, y))
  value <- matrix(1, nrow = dim[1], ncol = dim[2])
  rh_expr <- lu.create_const(value, dim)
