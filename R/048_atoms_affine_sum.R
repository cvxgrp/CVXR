...
SumEntries <- new_class("SumEntries", parent = AxisAffAtom, package = "CVXR",
  constructor = function(expr, axis = NULL, keepdims = FALSE) {
    ...
    obj <- .fast_new(SumEntries, S7_object(),
      id       = next_expr_id(),
...
## CVXPY SOURCE: sum.py:77-80 (#3080). Reduce the arg's bounds along the axis.
## CVXR axis is 1-based (axis=1 reduce columns, axis=2 reduce rows); the bounds
## reducer takes a numpy-style axis, so convert via numpy = 2 - cvxr_axis
## (cf. ADR D_CONV.1). Reshape the result to the atom's 2D shape.
method(bounds_from_args, SumEntries) <- function(x) {
  b <- get_bounds(.args(x)[[1L]])
  npaxis <- if (is.null(x@axis)) NULL else 2L - x@axis
  sb <- sum_bounds(b[[1L]], b[[2L]], axis = npaxis, keepdims = x@keepdims)
  lb <- sb[[1L]]; ub <- sb[[2L]]
...
method(format_labeled, SumEntries) <- function(x) {
