...
RelEntr <- new_class("RelEntr", parent = Elementwise, package = "CVXR",
  constructor = function(x, y, id = NULL) {
    ...
    obj <- .fast_new(RelEntr, S7_object(),
      id    = as.integer(id),
...
## CVXPY elementwise/rel_entr.py: rel_entr is smooth.
method(is_atom_smooth, RelEntr) <- function(x) TRUE
...
## CVXPY SOURCE: atoms/elementwise/rel_entr.py:69-93 (rel_entr._grad).
## rel_entr(x, y) = x * log(x/y); domain x > 0, y > 0.
## d/dx = log(x/y) + 1; d/dy = -x/y.
method(.grad, RelEntr) <- function(x, values, ...) {
  ...
  list(
    .elemwise_grad_to_diag(log(div) + 1, rows0, cols),
    .elemwise_grad_to_diag(-div,         rows1, cols)
  )
}
