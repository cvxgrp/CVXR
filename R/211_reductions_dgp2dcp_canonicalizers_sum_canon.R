...
.dgp_sum_canon <- function(expr, args) {
  ...
  if (is.null(expr@axis)) {
    ...
    result <- .dgp_add_canon(summation, .args(summation))
    canon <- result[[1L]]
    return(list(reshape_expr(canon, .shape(expr)), list()))
  }
