...
GenLambdaMax <- new_class("GenLambdaMax", parent = Atom, package = "CVXR",
  constructor = function(A, B, id = NULL) {
    ...
    obj <- .fast_new(GenLambdaMax, S7_object(),
      id    = as.integer(id),
...
## CVXPY SOURCE: atoms/gen_lambda_max.py:48-62 (gen_lambda_max._grad).
## CVXPY explicitly raises NotImplementedError. Mirror with list(NULL, NULL)
## (two args: A and B).
method(.grad, GenLambdaMax) <- function(x, values, ...) {
  list(NULL, NULL)
}
