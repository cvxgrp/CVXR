...
pnorm_exact_canon <- function(expr, args, solver_context = NULL) {
  ...
  ## p=2: direct SOC
  if (p == 2) {
    return(.pnorm_p2_canon(expr, args, solver_context = solver_context))
  }
...
.pnorm_p2_canon <- function(expr, args, solver_context = NULL) {
  x <- args[[1L]]
  axis <- expr@axis
  shape <- .shape(expr)
  bounds <- get_expr_bounds_if_supported(expr, solver_context)
...
pnorm_approx_canon <- function(expr, args, solver_context = NULL) {
  ...
  ## CVXPY SOURCE: pnorm_canon.py:121-137 -- filters constraints to SOCs
  ## only (the tower also contains an equality `sum(r) == t` and the abs
  ## decomposition, which shouldn't count toward num_soc).
  num_soc <- sum(vapply(constraints, function(c) .s7_is(c, SOC), logical(1L)))
  .maybe_warn_soc_approx(
    solver_context, target_cone = PowCone3D,
    approx_error = expr@approx_error, num_soc = num_soc,
    msg_head = sprintf("pnorm with p=%g is being approximated",
                       as.numeric(expr@original_p))
  )
