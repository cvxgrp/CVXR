...
ggplot(data = coefsNNLS, aes(x = as.factor(beta), y = estimate, fill = method)) +
  geom_bar(stat = "identity", position = "dodge") +
...
As with ordinary least squares, there are already R packages available which implement non-negative least squares, such as the package `nnls`. But that is actually an excellent demonstration of the power of `CVXR`: A single line of code here, namely `prob3.2$constraints <- list(beta >= 0)`, is doing the work of an entire package.


## 3.3 Support Vector Machines
TODO: WRITE UP BINARY SUPPORT VECTOR CLASSIFIER
