...
# -- .grad: subgradient of tr(X^-1) -------------------------------
## CVXPY SOURCE: atoms/tr_inv.py:89-108 (tr_inv._grad).
## d/dX tr(X^-1) = -X^{-T} X^{-T} = -(X^{-T})^2.
## NULL if min eigenvalue <= 0 (outside the open PSD cone).
method(.grad, TrInv) <- function(x, values, ...) {
