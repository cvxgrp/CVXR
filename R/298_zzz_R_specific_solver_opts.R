...
.build_solver_params <- function(solver_name, opts) {
  ...
  for (std_name in names(std_vals)) {
    ...
    if (is.null(info)) {
      ## No native translation for this solver: the value can reach nothing.
      ## Silently discarding it was the worst failure mode of the standard-name
      ## layer -- a user switching solver = "CLARABEL" to "MOSEK" lost feastol/
      ## reltol/abstol with no signal. Collect and warn ONCE below.
      dropped <- c(dropped, std_name)
      next
    }
  ...
  if (length(dropped) > 0L) {
    cli_warn(c(
      "{cli::qty(dropped)}Standard solver parameter{?s} {.arg {dropped}} {?is/are} not supported for solver {.val {solver_name}} and will be ignored.",
      "i" = "Pass the solver's native parameter name via {.fn psolve} / {.fn solver_opts} instead.",
      "i" = "See the mapping table returned by {.fn solver_default_param} and the solver parameters article."
