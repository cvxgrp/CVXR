...
partial_trace <- function(expr, dims, axis = 1L) {
  expr <- as_expr(expr)
  if (length(.shape(expr)) < 2L || .shape(expr)[1L] != .shape(expr)[2L]) {
    cli_abort("{.fn partial_trace} only supports 2-d square arrays.")
  }
  if (.shape(expr)[1L] != prod(dims)) {
  ...
  ## Propagate the argument's matrix structure to the result.
  ## CVXPY SOURCE: partial_trace.py:87-93 (CVXPY 1.9.2).
  ## Partial trace PRESERVES positive semidefiniteness, symmetry and
  ## Hermitian-ness, but the sum of Constant %*% expr %*% Constant terms
  ## cannot infer that on its own -- without the wrap every attribute came
  ## back FALSE and DCP analysis lost the PSD-ness of a PSD input.
  ## Order matters: PSD is the strongest claim, so it is tested first.
  if (is_psd(expr)) return(psd_wrap(result))
  if (is_symmetric(expr)) return(symmetric_wrap(result))
  if (is_hermitian(expr)) return(hermitian_wrap(result))
  result
}
