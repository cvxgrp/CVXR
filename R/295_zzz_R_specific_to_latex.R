...
.is_zero_constant <- function(expr) {
  ...
  if (!.s7_is(inner, Constant)) return(FALSE)
  v <- inner@.value
...
method(.to_latex_prec, AddExpression) <- function(x, names_map = NULL, ...) {
  ...
  for (i in seq_len(n)) {
    ...
    if (i == 1L) {
      ...
      ai <- x@args[[i]]
      neg_inner <- if (.s7_is(ai, NegExpression)) ai@args[[1L]]
        else if (.s7_is(ai, Promote) && .s7_is(ai@args[[1L]], NegExpression)) ai@args[[1L]]@args[[1L]]
        else NULL
...
method(.to_latex_prec, PSD) <- function(x, names_map = NULL, ...) {
  ...
  .unwrap_neg <- function(e) {
    if (.s7_is(e, NegExpression)) return(e)
    if (.s7_is(e, Promote) && .s7_is(e@args[[1L]], NegExpression)) return(e@args[[1L]])
    NULL
  }
  neg_node <- if (.s7_is(arg, AddExpression) && length(arg@args) == 2L)
    .unwrap_neg(arg@args[[2L]]) else NULL
  if (!is.null(neg_node)) {
    lhs_expr <- arg@args[[1L]]
    ## Unwrap Promote on LHS too (scalar LHS promoted by broadcast_args)
    if (.s7_is(lhs_expr, Promote)) lhs_expr <- lhs_expr@args[[1L]]
...
method(to_latex, Objective) <- function(x, ...) {
  sense <- if (.s7_is(x, Minimize)) "\\text{minimize}" else "\\text{maximize}"
...
method(to_latex, Problem) <- function(x, ...) {
  ...
  ## Objective
  obj <- x@objective
  sense <- if (.s7_is(obj, Minimize)) "mini" else "maxi"
