...
.dcp_diagnose_expr <- function(expr) {
  ...
  if (.s7_is(expr, Leaf)) return(NULL)
...
.viz_build_model <- function(problem, digits = 4L) {
  ...
  ## -- Build stage data ---------------------------------------------
  sense <- if (.s7_is(problem@objective, Minimize)) "minimize" else "maximize"
...
.viz_build_solver_data <- function(problem, solver = NULL, digits = 4L) {
  ...
  interface <- if (.s7_is(solver_reduction, QpSolver)) "qp" else "conic"
