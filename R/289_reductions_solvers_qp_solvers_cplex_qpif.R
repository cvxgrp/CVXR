...
## CVXPY SOURCE: cplex_qpif.py lines 31-35

CPLEX_QP_Solver <- new_class("CPLEX_QP_Solver", parent = QpSolver,
  package = "CVXR",
  constructor = function() {
    if (FALSE) new_object(S7_object())  ## S7 static-check guard
    .fast_new(CPLEX_QP_Solver, S7_object(),
...
method(solve_via_data, CPLEX_QP_Solver) <- function(x, data, warm_start = FALSE, verbose = FALSE,
  ...
  ## Variable bounds
  ## CVXPY SOURCE: cplex_qpif.py lines 113-126
...
method(reduction_invert, CPLEX_QP_Solver) <- function(x, solution, inverse_data, ...) {
  ...
  ## CVXPY SOURCE: cplex_qpif.py invert(), which calls the same
  ## `cplex_conif.get_status(model)`. Shared here too -- see `.cplex_get_status`
  ## in conic_solvers/cplex_conif.R for why a flat table could not do this.
