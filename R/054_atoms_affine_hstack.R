...
HStack <- new_class("HStack", parent = AffAtom, package = "CVXR",
  constructor = function(..., id = NULL) {
    ...
    rows <- .shape(all_args[[1L]])[1L]
    total_cols <- 0L
    for (arg in all_args) {
      if (.shape(arg)[1L] != rows) {
        cli_abort("All arguments to {.fn hstack} must have the same number of rows.")
      }
      total_cols <- total_cols + .shape(arg)[2L]
    }
    shape <- c(rows, total_cols)

    obj <- .fast_new(HStack, S7_object(),
      id    = as.integer(id),
...
method(shape_from_args, HStack) <- function(x) {
  rows <- .arg_shape(x)[1L]
  total_cols <- sum(vapply(.args(x), function(a) .shape(a)[2L], integer(1)))
  c(rows, total_cols)
}
