...
## CVXPY quad_over_lin.py: quad_over_lin is smooth.
method(is_atom_smooth, QuadOverLin) <- function(x) TRUE
...
## CVXPY SOURCE: atoms/quad_over_lin.py:73-103 (quad_over_lin._grad).
## Two args: X (vector / matrix) and y (scalar > 0).
## d/dX = 2X / y; d/dy = -||X||^2_2 / y^2. NULL on y <= 0 or with axis.
## Override .grad directly (rather than using .axis_grad) because the
## gradient is genuinely two-arg and the axis case is unsupported per
## CVXPY (returns [None, None]).
method(.grad, QuadOverLin) <- function(x, values, ...) {
  if (!is.null(x@axis)) return(list(NULL, NULL))
  X <- values[[1L]]
  y <- as.numeric(values[[2L]])
  if (y <= 0) return(list(NULL, NULL))

  ## d/dX = 2X / y, raveled column-major to match CVXPY's order='F'.
  DX <- 2 * as.numeric(X) / y
  ## d/dy = -||X||^2 / y^2, scalar.
