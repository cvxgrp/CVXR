...
method(param_backward, Dgp2Dcp) <- function(x, param, dparams) {
  dgp_methods <- x@.cache$dgp_methods
  if (is.null(dgp_methods)) return(NULL)
  pid <- as.character(param@id)
  if (!exists(pid, envir = dgp_methods$params_cache, inherits = FALSE)) return(NULL)
  log_param <- get(pid, envir = dgp_methods$params_cache, inherits = FALSE)
  new_pid <- as.character(log_param@id)
  if (is.null(dparams[[new_pid]])) return(NULL)
  ## d/d(param) = (1 / param) * d/d(log_param)
  (1 / value(param)) * dparams[[new_pid]]
}

method(param_forward, Dgp2Dcp) <- function(x, param, delta) {
  dgp_methods <- x@.cache$dgp_methods
  if (is.null(dgp_methods)) return(NULL)
  pid <- as.character(param@id)
  if (!exists(pid, envir = dgp_methods$params_cache, inherits = FALSE)) return(NULL)
  ...
  new_pid <- as.character(log_param@id)
  out <- list()
  out[[new_pid]] <- (1 / value(param)) * as.numeric(delta)
  out
}

## Generic uses `value` as the third arg name; match it here to keep
## S7 method-formal-match happy.
method(var_backward, Dgp2Dcp) <- function(x, var, value) {
  ## x = exp(log_var)  =>  dx/d(log_var) = x = var.value
  value * CVXR::value(var)
}

method(var_forward, Dgp2Dcp) <- function(x, var, value) {
  value * CVXR::value(var)
}
