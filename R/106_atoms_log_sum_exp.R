...
## CVXPY log_sum_exp.py: log_sum_exp is smooth.
method(is_atom_smooth, LogSumExp) <- function(x) TRUE
...
# -- .column_grad: softmax (numerically stable) -------------------
## CVXPY SOURCE: atoms/log_sum_exp.py:64-79 (log_sum_exp._column_grad).
## d/dx_i log(sum(exp(x))) = exp(x_i) / sum(exp(x_k)) = softmax.
## CVXPY uses scipy.special.logsumexp; we use a manual shift-and-sum
## for the same numerical stability.
method(.column_grad, LogSumExp) <- function(x, value, ...) {
  v <- as.numeric(value)
  m <- max(v)
  e <- exp(v - m)
  e / sum(e)
