...
perspective_canon <- function(expr, args, solver_context = NULL) {
  f <- expr@.f
  ...
  if (.s7_is(aux_prob@objective, Maximize)) {
  ...
  for (var in f_vars) {
    ...
    if (!is.null(var_offsets[[vid]])) {
      ...
      chunk_idx <- chunk_idx + 1L
      if (isTRUE(.attributes(var)$diag)) {
        ## Diagonal variable: extract diagonal entries.  After CvxAttr2Constr
        ## handles diag (cvx_attr2constr.R), x_canon allocates n columns for
        ## the diag variable (not n*n), and `diag(var)` (n entries) matches.
        constr_chunks[[chunk_idx]] <- list(
          diag(var) == x_canon[start_idx:end_idx]
        )
      } else if (expr_size(var) > 1L &&
                 (isTRUE(.attributes(var)$symmetric) ||
                  isTRUE(.attributes(var)$PSD) ||
                  isTRUE(.attributes(var)$NSD))) {
        ## Symmetric / PSD / NSD variable: upper triangle.  After
        ## CvxAttr2Constr, x_canon allocates n*(n+1)/2 columns for these
        ## variables.  PSD and NSD imply symmetry but do not auto-set the
        ## symmetric flag, so the original code (which checked symmetric
        ## only) missed them and fell through to the (n*n)-vector general
        ## case, producing a shape mismatch.
        n <- .shape(var)[1L]
