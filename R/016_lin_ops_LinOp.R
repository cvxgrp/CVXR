...
##
## DIRECT `.Call`, NOT the generated wrapper (ADR D_PERF.6): these run once per
## LinOp per node, so the wrapper's ~0.42us of pure closure overhead is a real
## share of the work. Each site names its generated equivalent in a
## `## wrapper:` comment, because `.Call` is positional, matches nothing by
## name, honors no defaults, and Rcpp SILENTLY COERCES a wrong type -- an
## allocation and a copy, i.e. exactly the cost being avoided. Arguments must
## therefore already have the right type at the call. `scripts/validate_rcpp_bridge.R`
## checks each comment against the real signature.
...
linop_set_type <- function(ptr, type) {
  index <- match(type, .LINOP_TYPES) - 1L   # integer: `match` returns integer
  ## wrapper: .LinOp__set_type(ptr, index)
...
linop_size_push_back <- function(ptr, value) {
  ## wrapper: .LinOp__size_push_back(ptr, value)
...
linop_args_push_back <- function(ptr, child_ptr) {
  ## wrapper: .LinOp__args_push_back(ptr, child_ptr)
...
linop_set_dense_data <- function(ptr, value) {
  ## wrapper: .LinOp__set_dense_data(ptr, value)
...
linop_set_sparse_data <- function(ptr, value) {
  ## wrapper: .LinOp__set_sparse_data(ptr, value)
...
linop_set_data_ndim <- function(ptr, value) {
  ## wrapper: .LinOp__set_data_ndim(ptr, as.integer(value))
...
linop_set_linop_data <- function(ptr, data_ptr) {
  ## wrapper: .LinOp__set_linOp_data(ptr, data_ptr)
...
linop_slice_push_back <- function(ptr, int_vector) {
  ## wrapper: .LinOp__slice_push_back(ptr, int_vector)
