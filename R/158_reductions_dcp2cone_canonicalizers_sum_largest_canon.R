...
sum_largest_canon <- function(expr, args, solver_context = NULL) {
  ...
  if (is.null(axis)) {
    ...
    if (!is.null(xval)) {
      ...
      value(t) <- matrix(tv, nrow = x@shape[1L], ncol = x@shape[2L])
    }
  }
