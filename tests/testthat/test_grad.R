...
test_that("Test gradient for Pnorm", {
  value(x) <- c(-1,0)
  expr <- Pnorm(x, 1)
  expect_equal(as.matrix(grad(expr)[[as.character(x@id)]]), as.matrix(c(-1,0)), tolerance = TOL)
  
  value(x) <- c(0,10)
  expr <- Pnorm(x, 1)
  expect_equal(as.matrix(grad(expr)[[as.character(x@id)]]), as.matrix(c(0,1)), tolerance = TOL)
  
  value(x) <- c(-3,4)
  expr <- Pnorm(x, 2)
  expect_equal(as.matrix(grad(expr)[[as.character(x@id)]]), as.matrix(c(-3.0/5),4.0/5), tolerance = TOL)
  ...
  value(x) <- c(0,0)
  expr <- Pnorm(x, 2)
  expect_equal(as.matrix(grad(expr)[[as.character(x@id)]]), as.matrix(c(0,0)))
  
  value(A) <- rbind(c(2,-2), c(2,2))
  expr <- Pnorm(A, 2)
  expect_equal(as.matrix(grad(expr)[[as.character(A@id)]]), matrix(c(0.5,0.5,-0.5,0.5)))
  
  value(A) <- rbind(c(3,-3), c(4,4))
  expr <- Pnorm(A, 2, axis = 2)
  expect_equal(as.matrix(grad(expr)[[as.character(A@id)]]), rbind(c(0.6,0), c(0.8,0), c(0,-0.6), c(0,0.8)))
  
  value(A) <- rbind(c(3,-4), c(4,3))
  expr <- Pnorm(A, 2, axis = 1)
  expect_equal(as.matrix(grad(expr)[[as.character(A@id)]]), rbind(c(0.6,0), c(0,0.8), c(-0.8,0), c(0,0.6)))
...
test_that("Test gradient for LogSumExp", {
  ...
  expr <- LogSumExp(x)
  e <- exp(1)
  expect_equal(as.matrix(grad(expr)[[as.character(x@id)]]), c(1.0/(1+e), e/(1+e)))
  
  value(A) <- rbind(c(0,1), c(-1,0))
  expr <- LogSumExp(A)
  expect_equal(as.matrix(grad(expr)[[as.character(A@id)]]), c(1.0/(2+e+1.0/e), 1.0/e/(2+e+1.0/e), e/(2+e+1.0/e), 1.0/(2+e+1.0/e)))
  
  value(A) <- rbind(c(0,1), c(-1,0))
  expr <- LogSumExp(A, axis = 2)
  expect_equal(as.matrix(grad(expr)[[as.character(A@id)]]), cbind(c(1.0/(1+1.0/e), 1.0/e/(1+1.0/e), 0, 0), c(0, 0, e/(1+e), 1.0/(1+e))))
})

test_that("Test gradient for GeoMean", {
  value(x) <- c(1,2)
  expr <- GeoMean(x)
  expect_equal(as.matrix(grad(expr)[[as.character(x@id)]]), c(sqrt(2)/2, 1.0/2/sqrt(2)))
  ...
  expect_equal(as.matrix(grad(expr)[[as.character(x@id)]]), c(1,0))
...
test_that("Test gradient for LambdaMax", {
  ...
  expr <- LambdaMax(A)
  expect_equal(as.matrix(grad(expr)[[as.character(A@id)]]), c(1,0,0,0))
  
  value(A) <- rbind(c(1,0), c(0,2))
  expr <- LambdaMax(A)
  expect_equal(as.matrix(grad(expr)[[as.character(A@id)]]), c(0,0,0,1))
...
test_that("Test gradient for MatrixFrac", {
  ...
  expr <- MatrixFrac(A, B)
  expect_equal(as.matrix(grad(expr)[[as.character(A@id)]]), c(2,0,0,2))
  expect_equal(as.matrix(grad(expr)[[as.character(B@id)]]), c(-1,0,0,-1))
  
  value(B) <- matrix(0, nrow = 2, ncol = 2)
  expr <- MatrixFrac(A, B)
  expect_equal(grad(expr)[[as.character(A@id)]], NA)
  expect_equal(grad(expr)[[as.character(B@id)]], NA)
  
  value(x) <- c(2,3)
  value(A) <- diag(rep(1,2))
  expr <- MatrixFrac(x, A)
  expect_equal(as.matrix(grad(expr)[[as.character(x@id)]]), c(4,6))
  expect_equal(as.matrix(grad(expr)[[as.character(A@id)]]), c(-4,-6,-6,-9))
})

test_that("Test gradient for NormNuc", {
  value(A) <- rbind(c(10,4), c(4,30))
  expr <- NormNuc(A)
  expect_equal(as.matrix(grad(expr)[[as.character(A@id)]]), c(1,0,0,1))
})

test_that("Test gradient for LogDet", {
  value(A) <- 2*diag(rep(1,2))
  expr <- LogDet(A)
  expect_equal(as.matrix(grad(expr)[[as.character(A@id)]]), 1.0/2*diag(rep(1,2)))
...
test_that("Test gradient for QuadOverLin", {
  ...
  expect_equal(as.matrix(grad(expr)[[as.character(x@id)]]), c(1,2))
...
test_that("Test gradient for MaxEntries", {
  ...
  expect_equal(as.matrix(grad(expr)[[as.character(A@id)]]), c(0,1,0,0))
  
  expr <- MaxEntries(A, axis = 0)
  value(A) <- rbind(c(1,2), c(4,3))
  expect_equal(as.matrix(grad(expr)[[as.character(A@id)]]), rbind(c(0,0), c(1,0), c(0,0), c(0,1)))
  
  expr <- MaxEntries(A, axis = 1)
  value(A) <- rbind(c(1,2), c(4,3))
  expect_equal(as.matrix(grad(expr)[[as.character(A@id)]]), rbind(c(0,0), c(0,1), c(1,0), c(0,0)))
})
...
test_that("Test SumLargest", {
  ...
  expect_equal(as.matrix(grad(expr)[[as.character(A@id)]]), c(1,0,1,0))
  
  value(A) <- rbind(c(1,2), c(3,0.5))
  expect_equal(as.matrix(grad(expr)[[as.character(A@id)]]), c(0,1,1,0))
})
...
test_that("Test grad for partial minimization/maximization problems", {
  for(obj in list(Minimize(a^-1), Maximize(Entr(a)))) {
    ...
    value(x) <- NA
    grad <- grad(expr)
    expect_equal(grad[[as.character(a@id)]], NA)
    expect_equal(grad[[as.character(x@id)]], NA)
