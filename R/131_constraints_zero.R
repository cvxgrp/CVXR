...
##  - DNLP predicates mirror zero.py:61 and zero.py:140: Zero and Equality are
##    DNLP exactly when their constrained expression is smooth-representable.
...
## CVXPY SOURCE: zero.py lines 61-63
method(is_dnlp, Zero) <- function(x) is_smooth(x@args[[1L]])
...
# -- is_dnlp ------------------------------------------------------
## An equality constraint is DNLP if (lhs - rhs) is smooth representable.
## CVXPY SOURCE: zero.py lines 140-142
method(is_dnlp, Equality) <- function(x) is_smooth(x@.expr)
