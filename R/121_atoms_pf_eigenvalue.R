...
PfEigenvalue <- new_class("PfEigenvalue", parent = Atom, package = "CVXR",
  constructor = function(X, id = NULL) {
    ...
    obj <- .fast_new(PfEigenvalue, S7_object(),
      id    = as.integer(id),
...
## CVXPY SOURCE: atoms/pf_eigenvalue.py:101-102 (pf_eigenvalue._grad).
## CVXPY returns [None]. Mirror with list(NULL).
method(.grad, PfEigenvalue) <- function(x, values, ...) {
  list(NULL)
}
