...
method(bounds_from_args, AddExpression) <- function(x) {
  ...
  bb <- broadcast_bounds(b[[1L]], b[[2L]], x@shape)
  lb <- bb[[1L]]; ub <- bb[[2L]]
  for (arg in x@args[-1L]) {
    ab <- get_bounds(arg)
    ab <- broadcast_bounds(ab[[1L]], ab[[2L]], x@shape)
...
method(format_labeled, AddExpression) <- function(x) {
  ...
  paste(vapply(x@args, format_labeled, character(1)), collapse = " + ")
}
