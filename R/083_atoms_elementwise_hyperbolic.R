...
# -- domain: -1 < x < 1 (hyperbolic.py:198-199) -------------------
## CVXPY expresses this with STRICT inequalities (`x < 1`, `x > -1`).  In
## CVXPY 1.9.0 strict inequalities raise (expression.py:993/1000,
## "Strict inequalities are not allowed."), so atanh._domain is itself
## unevaluable upstream.  CVXR mirrors that exactly: the open domain (-1, 1)
## is not representable as a non-strict Inequality, so we abort here too.
method(atom_domain, Atanh) <- function(x) {
  cli_abort(c(
    "Domain of {.fn atanh} requires strict inequalities, which are not allowed.",
    "i" = "The domain is the open interval {.code (-1, 1)}, not representable as a CVXR constraint."
  ))
}
