...
##  - DNLP linearizable predicates mirror indicator.py:59-62: Indicator is
##    linearizable-convex and not linearizable-concave.
...
## DNLP curvature. CVXPY SOURCE: indicator.py lines 59-62
method(is_linearizable_convex, Indicator) <- function(x) TRUE
method(is_linearizable_concave, Indicator) <- function(x) FALSE
...
method(expr_name, Indicator) <- function(x) {
  arg_names <- vapply(x@args, function(c) {
    if (.s7_is(c, Canonical)) expr_name(c) else as.character(c)
  }, character(1))
