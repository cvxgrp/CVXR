...
c2r_variable_canon <- function(expr, real_args, imag_args, real2imag) {
  if (is_real(expr)) {
    ...
    ## Purely imaginary
    imag_var <- Variable(.shape(expr), var_id = real2imag[[as.character(.id(expr))]])
    ...
    n <- .shape(expr)[1L]
    real_var <- Variable(c(n, n), var_id = .id(expr), symmetric = TRUE)
    if (n > 1L) {
      imag_var <- Variable(c((n * (n - 1L)) %/% 2L, 1L),
                           var_id = real2imag[[as.character(.id(expr))]])
    ...
    ## General complex
    real_var <- Variable(.shape(expr), var_id = .id(expr))
    imag_var <- Variable(.shape(expr), var_id = real2imag[[as.character(.id(expr))]])
