...
AddExpression <- new_class("AddExpression", parent = AffAtom, package = "CVXR",
  constructor = function(arg_groups) {
    ## NEW_OBJECT GUARD: S7's check_S7_constructor static-checks that the
    ## body contains a literal new_object() call.  This branch never runs;
    ## all actual construction goes through .fast_new (Lever 2 experiment).
    ...
    if (length(arg_groups) == 2L) {
      g1 <- arg_groups[[1L]]
      g2 <- arg_groups[[2L]]
      if (!.s7_is(g1, Expression)) g1 <- as_expr(g1)
      if (!.s7_is(g2, Expression)) g2 <- as_expr(g2)
      is_add1 <- .s7_is(g1, AddExpression)
      is_add2 <- .s7_is(g2, AddExpression)
      if (is_add1 && !is_add2) {
        flat_args <- c(.args(g1), list(g2))
      } else if (!is_add1 && is_add2) {
        flat_args <- c(list(g1), .args(g2))
      } else if (is_add1 && is_add2) {
        flat_args <- c(.args(g1), .args(g2))
      } else {
      ...
      shape <- .broadcast_2d(.shape(g1), .shape(g2))
      return(.fast_new(AddExpression, S7_object(),
        id    = next_expr_id(),
    ...
    for (i in seq_len(n_groups)) {
      ...
      if (.s7_is(group, AddExpression)) {
        flat_chunks[[i]] <- .args(group)
      } else {
    ...
    shape <- sum_shapes(lapply(flat_args, function(a) .shape(a)))
    .fast_new(AddExpression, S7_object(),
      id    = next_expr_id(),
      .cache = new.env(parent = emptyenv()),
      args  = flat_args,
      shape = shape
...
# -- shape_from_args -------------------------------------------------
## CVXPY SOURCE: add_expr.py:86-97 (#3080). Broadcast each arg's bounds to the
## sum's shape, then add interval-wise.
method(bounds_from_args, AddExpression) <- function(x) {
  b <- get_bounds(.args(x)[[1L]])
  bb <- broadcast_bounds(b[[1L]], b[[2L]], .shape(x))
  lb <- bb[[1L]]; ub <- bb[[2L]]
  for (arg in .args(x)[-1L]) {
    ab <- get_bounds(arg)
    ab <- broadcast_bounds(ab[[1L]], ab[[2L]], .shape(x))
    s <- add_bounds(lb, ub, ab[[1L]], ab[[2L]])
    lb <- s[[1L]]; ub <- s[[2L]]
  }
  list(lb, ub)
}
...
method(expr_name, AddExpression) <- function(x) {
  paste(vapply(.args(x), expr_name, character(1)), collapse = " + ")
}

## CVXPY SOURCE: atoms/affine/add_expr.py AddExpression.format_labeled.
method(format_labeled, AddExpression) <- function(x) {
  lbl <- label(x)
  if (!is.null(lbl)) return(lbl)
  paste(vapply(.args(x), format_labeled, character(1)), collapse = " + ")
}
...
method(expr_copy, AddExpression) <- function(x, args = NULL, id_objects = NULL) {
  ## Memo skipped when `id_objects` is NULL -- it could not hit. Same reasoning
  ## and the same measurement as method(expr_copy, Canonical); see
  ## utilities/canonical.R.
