...
test_that("Test domain for partial minimization/maximization problems", {
  ...
  for(obj in list(Minimize(a^(-1)), Maximize(log(a)))) {
    ...
    expect_true(all(result$getValue(x + a - c(5, 8)) >= -1e-3))

    # Optimize over x.
    expr <- partial_optimize(orig_prob, opt_vars = list(x))
    dom <- domain(expr)
    constr <- list(a >= -100, x >= 0)
    prob <- Problem(Minimize(sum(x + a)), c(dom, constr))
    result <- solve(prob, solver = "ECOS")
    expect_equal(result$value, 0, tolerance = TOL)
    expect_true(result$getValue(a) >= 1e-3)
    expect_equal(result$getValue(x), matrix(c(0,0)), tolerance = TOL)

    # Optimize over x and a.
    expr <- partial_optimize(orig_prob, opt_vars = list(x, a))
    dom <- domain(expr)
    constr <- list(a >= -100, x >= 0)
    prob <- Problem(Minimize(sum(x + a)), c(dom, constr))
    result <- solve(prob, solver = "ECOS")
...
test_that("Test domain for geo_mean", {
  ...
  result <- solve(prob, solver = "SCS", eps = 1e-5)
  expect_equal(result$getValue(z), matrix(c(-1,0,0)), tolerance = TOL)
})

test_that("Test domain for quad_over_lin", {
  skip_on_cran()
  dom <- domain(quad_over_lin(x, a))
  result <- solve(Problem(Minimize(a), dom), solver = "SCS", eps = 1e-6)
  expect_equal(result$getValue(a), 0, tolerance = TOL)
})

test_that("Test domain for lambda_max", {
  print("Throws Segfault from Eigen. Skipping test.")
  return()
  
  skip_on_cran()
  dom <- domain(lambda_max(A))
  A0 <- rbind(c(1,2), c(3,4))
  result <- solve(Problem(Minimize(norm2(A - A0)), dom), solver = "SCS")
...
test_that("Test domain for kl_div", {
  ...
  result <- solve(Problem(Minimize(a + b), dom), solver = "SCS", eps = 1e-5)
  expect_equal(result$getValue(a), 0, tolerance = TOL)
...
test_that("Test domain for power", {
  skip_on_cran()
  dom <- domain(sqrt(a))
  result <- solve(Problem(Minimize(a), dom), solver = "SCS", eps = 1e-8, max_iters = 1e5)
  expect_equal(result$getValue(a), 0, tolerance = TOL)

  # dom <- domain(a^2)
  dom <- domain(square(a))
  result <- solve(Problem(Minimize(a), c(dom, a >= -100)), solver = "SCS", eps = 1e-8, max_iters = 1e5)
  expect_equal(result$getValue(a), -100, tolerance = TOL)
...
test_that("Test domain for matrix_frac", {
  skip_on_cran()
  dom <- domain(matrix_frac(x, A + diag(2)))
  prob <- Problem(Minimize(sum(diag(A))), dom)
  result <- solve(prob, solver = "SCS")
  expect_equal(result$value, -2, tolerance = 1e-3)
})
