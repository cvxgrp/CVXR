...
##  - DNLP predicates mirror nonpos.py:71-76, 137-142, and 227-232:
##    NonPos/Inequality require a linearizable-convex expression; NonNeg
##    requires a linearizable-concave expression.
...
# -- is_dnlp ------------------------------------------------------
## A NonPos constraint is DNLP if its argument is linearizable convex.
## CVXPY SOURCE: nonpos.py lines 71-76
method(is_dnlp, NonPos) <- function(x) is_linearizable_convex(x@args[[1L]])
...
# -- is_dnlp ------------------------------------------------------
## A NonNeg constraint is DNLP if its argument is linearizable concave.
## CVXPY SOURCE: nonpos.py lines 137-142
method(is_dnlp, NonNeg) <- function(x) is_linearizable_concave(x@args[[1L]])
...
# -- is_dnlp ------------------------------------------------------
## An inequality is DNLP if (lhs - rhs) is linearizable convex.
## CVXPY SOURCE: nonpos.py lines 227-232
method(is_dnlp, Inequality) <- function(x) is_linearizable_convex(x@.expr)
