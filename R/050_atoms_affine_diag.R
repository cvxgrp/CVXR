...
DiagVec <- new_class("DiagVec", parent = AffAtom, package = "CVXR",
  ...
  constructor = function(x, k = 0L, id = NULL) {
    ...
    obj <- .fast_new(DiagVec, S7_object(),
      id    = as.integer(id),
...
method(shape_from_args, DiagVec) <- function(x) {
  n <- .arg_shape(x)[1L] + abs(x@k)
  c(n, n)
}

## CVXPY: diag.py lines 96-104
method(is_symmetric, DiagVec) <- function(x) x@k == 0L
## CVXPY v1.9.0 fix: #3180 -- diag(v) is Hermitian only when v is real
## (a diagonal of complex entries is symmetric but not Hermitian).
method(is_hermitian, DiagVec) <- function(x) x@k == 0L && is_real(.args(x)[[1L]])
...
method(numeric_value, DiagVec) <- function(x, values, ...) {
  ...
  m <- matrix(zero, nrow = .shape(x)[1L], ncol = .shape(x)[2L])
  n <- length(v)
  if (x@k >= 0L) {
...
DiagMat <- new_class("DiagMat", parent = AffAtom, package = "CVXR",
  ...
  constructor = function(x, k = 0L, id = NULL) {
    ...
    obj <- .fast_new(DiagMat, S7_object(),
      id    = as.integer(id),
      .cache = new.env(parent = emptyenv()),
      args  = list(x),
      shape = shape,
...
method(shape_from_args, DiagMat) <- function(x) {
  c(.arg_shape(x)[1L] - abs(x@k), 1L)
}

## CVXPY: diag.py lines 178-180
method(is_nonneg, DiagMat) <- function(x) {
  (is_nonneg(.args(x)[[1L]]) || is_psd(.args(x)[[1L]])) && x@k == 0L
}
