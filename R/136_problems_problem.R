...
psolve <- function(problem, solver = NULL, gp = FALSE, qcp = FALSE,
                   ...
                   requires_grad = FALSE, solver_path = NULL, ...) {
...
backward <- function(problem) {
  ...
  ## Build del_vars: var-id -> gradient (NULL on a Variable means the
  ## all-ones vector; matches CVXPY's "default sum-of-x loss").
  del_vars <- list()
  for (v in variables(problem)) {
    vid_str <- as.character(v@id)
    g <- gradient(v)
    if (is.null(g)) g <- array(1, dim = v@shape)
    g <- as.numeric(g)
    ## Walk reductions in chain order, applying var_backward.
    for (red in reductions) {
      g <- var_backward(red, v, g)
    }
    del_vars[[vid_str]] <- g
  }
  ...
  ## Per parameter: start with the direct contribution if the param
  ## passed through unchanged; then accumulate chain-rule contributions
  ## from each reduction's param_backward.
  for (p in parameters(problem)) {
    pid_str <- as.character(p@id)
    grad <- if (!is.null(dparams[[pid_str]])) dparams[[pid_str]]
            else array(0, dim = p@shape)
    for (red in reductions) {
      contrib <- param_backward(red, p, dparams)
      if (!is.null(contrib)) grad <- grad + contrib
    }
    gradient(p) <- grad
  }
...
derivative <- function(problem) {
  ...
  ## Build per-id parameter deltas: direct entry if the param
  ## passed through unchanged, plus contributions from each
  ## reduction's param_forward.
  param_deltas <- list()
  for (p in parameters(problem)) {
    pid_str <- as.character(p@id)
    d <- delta(p)
    if (is.null(d)) d <- array(0, dim = p@shape)
    if (!is.null(param_prog@param_id_to_col[[pid_str]])) {
      param_deltas[[pid_str]] <- as.numeric(d)
    }
    for (red in reductions) {
      contrib <- param_forward(red, p, d)
      if (!is.null(contrib)) {
        for (k in names(contrib)) param_deltas[[k]] <- as.numeric(contrib[[k]])
      }
    }
  }

  ## Convert per-id deltas into (dc, dA, db) by re-running the same
  ## parameter -> data tensor multiply, with parameter values
  ## *replaced* by deltas and the constant offset zeroed out.
  ...
  for (v in variables(problem)) {
    vid_str <- as.character(v@id)
    dv <- if (!is.null(dvars[[vid_str]])) dvars[[vid_str]]
          else array(0, dim = v@shape)
    for (red in reductions) {
      dv <- var_forward(red, v, dv)
    }
    delta(v) <- dv
  }
