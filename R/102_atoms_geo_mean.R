...
GeoMean <- new_class("GeoMean", parent = Atom, package = "CVXR",
  ...
  constructor = function(x, p = NULL, max_denom = 1024L, id = NULL) {
    ...
    obj <- .fast_new(GeoMean, S7_object(),
      id           = as.integer(id),
...
## CVXPY geo_mean.py: geo_mean is smooth.
method(is_atom_smooth, GeoMean) <- function(x) TRUE
...
## CVXPY SOURCE: atoms/geo_mean.py:259-281 (geo_mean._grad).
## D[i] = w[i] / x[i] * geo_mean(x; w), elementwise.
## Domain: returns NULL if any x[i] <= 0 where w[i] > 0.
## w may be bigq (rational approx via PowerApprox-like machinery);
## coerce via as.numeric, mirroring CVXPY's float(w_i) cast on line 268.
method(.grad, GeoMean) <- function(x, values, ...) {
  v <- as.numeric(values[[1L]])     # column-major flatten
  w_arr <- as.numeric(x@w)
...
GeoMeanApprox <- new_class("GeoMeanApprox", parent = GeoMean, package = "CVXR",
  ...
  constructor = function(x, p = NULL, max_denom = 1024L, id = NULL) {
    ...
    obj <- .fast_new(GeoMeanApprox, S7_object(),
      id            = as.integer(id),
      .cache        = new.env(parent = emptyenv()),
      args          = list(x),
      shape         = shape,
