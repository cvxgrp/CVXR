...
## R-SPECIFIC: Public API exports for CVXR
##
## A running record of what each build phase exported, kept as comments; the
## actual exporting is done by roxygen tags on the definitions themselves and
## lands in NAMESPACE.
...
installed_solvers <- function() {
  nms <- names(.SOLVER_PACKAGES)
  nms[vapply(nms, .solver_package_usable, logical(1L), USE.NAMES = FALSE)]
}
...
available_solvers <- function() {
  nms <- names(.SOLVER_PACKAGES)
  nms[vapply(nms, .solver_package_available, logical(1L), USE.NAMES = FALSE)]
}
