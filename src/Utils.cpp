...
// Tensor algebra utilities for DPP support.
// Ported from CVXPY cvxcore/src/Utils.cpp
...
int vecprod_before(const std::vector<int> &vec, int end) {
  return std::accumulate(vec.rbegin() + vec.size() - end, vec.rend(), 1,
                         std::multiplies<int>());
}

// Multiply two vectors of matrices, get a new vector of matrices.
// One of the vectors must have size 1 (broadcast multiplication).
...
// Accumulate right hand vector of matrices into left hand by addition.
...
// Multiply two DictMat structures.
// Left hand side keys are always CONSTANT_ID.
...
// Accumulate right hand DictMat into left hand by addition.
...
// Tensor multiplication. Enforces DPP rule: no cross-parameter terms
// (one of lh_param_id or rh_param_id must be CONSTANT_ID).
Tensor tensor_mul(const Tensor &lh_ten, const Tensor &rh_ten) {
  ...
  for (auto it = lh_ten.begin(); it != lh_ten.end(); ++it) {
    ...
    for (auto jit = rh_ten.begin(); jit != rh_ten.end(); ++jit) {
      ...
      // No cross terms allowed (DPP rule).
...
// Create a diagonal matrix with entries given by the entries of mat.
