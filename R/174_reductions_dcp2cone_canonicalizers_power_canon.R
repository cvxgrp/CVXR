...
power_approx_canon <- function(expr, args, solver_context = NULL) {
  ...
  ## CVXPY SOURCE: power_canon.py:84-101 -- warn when the chosen solver
  ## actually supports power cones and either the approximation error is
  ## above 1e-6 or the SOC tower has more than 4 constraints.
  .maybe_warn_soc_approx(
    solver_context, target_cone = PowCone3D, approx_error = expr@approx_error,
    num_soc = length(constrs),
    msg_head = sprintf("Power atom with exponent %g is being approximated with rational %g",
                       as.numeric(expr@p_orig), as.numeric(p))
  )
