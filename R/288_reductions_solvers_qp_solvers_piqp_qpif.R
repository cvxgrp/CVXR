...
method(solve_via_data, PIQP_QP_Solver) <- function(x, data, warm_start = FALSE, verbose = FALSE,
                                                     solver_opts = list(), ...) {
  .require_solver_package(PIQP_SOLVER)
  ...
  F_ineq <- data[["F_ineq"]]
  g_ineq <- data[["g_ineq"]]
  x_l <- data[[LOWER_BOUNDS]]
  x_u <- data[[UPPER_BOUNDS]]
  ...
  if (warm_start && !is.null(solver_cache) && exists(cache_key, envir = solver_cache)) {
    ...
    ## Check vector changes: q (c), b_eq (b), g_ineq (h_u), variable bounds.
    ...
    ## CVXPY SOURCE: piqp_qpif.py lines 134-142
    if (!identical(x_l, old_data$x_l))        new_args[["x_l"]] <- x_l
    if (!identical(x_u, old_data$x_u))        new_args[["x_u"]] <- x_u
  ...
  ## -- Cold path ----------------------------------------------------
  ## CVXPY SOURCE: piqp_qpif.py lines 154-186
  if (!used_warm) {
    model <- piqp::piqp(
      P        = P,
      c        = q_vec,
